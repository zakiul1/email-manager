<div class="space-y-6 p-6">
    <section>
        <div class="border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            <div
                class="mb-6 flex flex-col gap-4 border-b border-zinc-200 pb-4 dark:border-zinc-700 sm:flex-row sm:items-center sm:justify-between">
                <div>
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('sendportal.workspace.campaigns.index') }}" wire:navigate
            ...
            <form id="campaign-form" wire:submit="save" class="space-y-6">
                <div class="grid gap-6 lg:grid-cols-2">
                    <div>
                        <label class="mb-2 block text-sm font-medium text-zinc-800 dark:text-zinc-100">Campaign
                            Name</label>
                ...
                <div class="grid gap-6 md:grid-cols-3">
                    <div>
                        ...
                        <input type="text" wire:model="preheader"
                            class="w-full border border-zinc-300 bg-white px-4 py-2.5 text-sm outline-none focus:border-zinc-500 focus:ring-0 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        @error('preheader')
                            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-medium text-zinc-800 dark:text-zinc-100">Delivery
                            Mode</label>
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select wire:model="smtp_pool_id"
                            class="w-full border border-zinc-300 bg-white px-4 py-2.5 text-sm outline-none focus:border-zinc-500 focus:ring-0 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                ...
                <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-medium text-zinc-800 dark:text-zinc-100">From
                            Email</label>
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-medium text-zinc-800 dark:text-zinc-100">Reply-To
                            Name</label>
                        <input type="text" wire:model="reply_to_name"
                            class="w-full border border-zinc-300 bg-white px-4 py-2.5 text-sm outline-none focus:border-zinc-500 focus:ring-0 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        @error('reply_to_name')
                            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-medium text-zinc-800 dark:text-zinc-100">Reply-To
                            Email</label>
                        <input type="email" wire:model="reply_to_email"
                            class="w-full border border-zinc-300 bg-white px-4 py-2.5 text-sm outline-none focus:border-zinc-500 focus:ring-0 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                ...
                <div class="border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800/50">
                    <div class="mb-3">
                        <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">Send Test Mail</h2>
                        <p class="mt-1 text-xs text-zinc-600 dark:text-zinc-300">
                            Send a test email through the selected SMTP pool using this campaign’s subject and content.
                        </p>
                    </div>

                    <div class="grid gap-4 md:grid-cols-[1fr_auto]">
                        <div>
                            <label class="mb-2 block text-sm font-medium text-zinc-800 dark:text-zinc-100">Test
                                Recipient Email</label>
                            <input type="email" wire:model.defer="test_recipient_email" placeholder="test@example.com"
                                class="w-full border border-zinc-300 bg-white px-4 py-2.5 text-sm outline-none focus:border-zinc-500 focus:ring-0 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                            @error('test_recipient_email')
                                <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                            @enderror

                            @unless ($campaign?->exists)
                                <div class="mt-1 text-xs text-amber-600 dark:text-amber-400">
                                    Save the campaign first, then you can send a test mail.
                                </div>
                            @endunless
                        </div>

                        <div class="flex items-end">
                            <button type="button" wire:click="sendTestMail" wire:loading.attr="disabled"
                                wire:target="sendTestMail" @disabled(!$campaign?->exists)
                                class="inline-flex items-center justify-center bg-zinc-900 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-zinc-700 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200">
                                <span wire:loading.remove wire:target="sendTestMail">Send Test Mail</span>
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    <div>
                        <label
                            class="mb-2 block text-sm font-medium text-zinc-800 dark:text-zinc-100">Categories</label>

                        <input type="text" wire:model.live.debounce.300ms="category_search"
                        ...
                        <div
                            class="h-64 overflow-y-auto border border-zinc-300 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                            @forelse ($categories as $item)
                                @php($checked = in_array((string) $item->id, $audience_ids, true))
                                <button type="button" wire:click="toggleAudience('{{ $item->id }}')"
                                    ...
                                    <span
                                        class="text-sm {{ $checked ? 'font-medium text-zinc-950 dark:text-white' : 'text-zinc-700 dark:text-zinc-200' }}">
                                        {{ $item->name }}
                                    </span>

                                    <span
                                        class="ml-3 inline-flex h-5 w-5 items-center justify-center border text-xs {{ $checked ? 'border-zinc-900 bg-zinc-900 text-white dark:border-white dark:bg-white dark:text-zinc-900' : 'border-zinc-300 text-transparent dark:border-zinc-600' }}">
                                        ✓
                                    </span>
                                </button>
                            @empty
                        ...
                        <div
                            class="mt-3 border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-800/50">
                            ...
                            <div class="mt-2 flex flex-wrap gap-2">
                                @forelse ($categories->whereIn('id', collect($audience_ids)->map(fn ($id) => (int) $id)->all()) as $selectedCategory)
                                    <button type="button" wire:click="toggleAudience('{{ $selectedCategory->id }}')"
                                ...
                                @empty
                                    <span class="text-sm text-zinc-500 dark:text-zinc-400">No categories
                                        selected.</span>
                                @endforelse
                            </div>
                        </div>
                    ...
                    @if ($delivery_mode === 'schedule')
                        <div>
                            <label class="mb-2 block text-sm font-medium text-zinc-800 dark:text-zinc-100">Scheduled
                                At</label>
                ...
                <div>
                    ...
                    <textarea wire:model="html_content" rows="12"
                        class="w-full border border-zinc-300 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-500 focus:ring-0 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white"></textarea>
                    @error('html_content')
                        <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    <label class="mb-2 block text-sm font-medium text-zinc-800 dark:text-zinc-100">Plain Text
                        Body</label>
                    <textarea wire:model="text_content" rows="8"
                        class="w-full border border-zinc-300 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-500 focus:ring-0 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white"></textarea>
                    @error('text_content')
                        <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                    @enderror
