                    ...
                    <tbody class="divide-y divide-zinc-200 bg-white dark:divide-zinc-700 dark:bg-zinc-900">
                        @forelse ($templates as $template)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-300">{{ $template->subject }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-300">{{ number_format($template->usage_count) }}</td>
                                ...
                                <td class="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-300">{{ $template->updated_at?->diffForHumans() }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('sendportal.workspace.templates.preview', $template) }}" wire:navigate class="border border-zinc-300 px-3 py-2 text-xs font-medium text-zinc-900 hover:bg-zinc-50 dark:border-zinc-700 dark:text-white dark:hover:bg-zinc-800">Preview</a>
                                        <a href="{{ route('sendportal.workspace.templates.test-send', $template) }}" wire:navigate class="border border-sky-300 px-3 py-2 text-xs font-medium text-sky-700 hover:bg-sky-50 dark:border-sky-800 dark:text-sky-300 dark:hover:bg-sky-950">Test Send</a>
                                        <button type="button" wire:click="duplicateTemplate({{ $template->id }})" class="border border-zinc-300 px-3 py-2 text-xs font-medium text-zinc-900 hover:bg-zinc-50 dark:border-zinc-700 dark:text-white dark:hover:bg-zinc-800">Duplicate</button>
                                        <a href="{{ route('sendportal.workspace.templates.edit', $template) }}" wire:navigate class="border border-zinc-300 px-3 py-2 text-xs font-medium text-zinc-900 hover:bg-zinc-50 dark:border-zinc-700 dark:text-white dark:hover:bg-zinc-800">Edit</a>
                                        <button type="button" wire:click="deleteTemplate({{ $template->id }})" wire:confirm="Are you sure you want to delete this template?" class="border border-red-300 px-3 py-2 text-xs font-medium text-red-700 hover:bg-red-50 dark:border-red-800 dark:text-red-300 dark:hover:bg-red-950">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
